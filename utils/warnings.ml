...
let descriptions = [
  ...
  { number = 75;
    ...
    since = since 5 4 };
  { number = 76;
    names = ["module-compiled-without-lto"];
    description = "A -use-lto link met a module without stored \
                   whole-program code.";
    since = since 5 4 };
]
...
let message = function
  ...
  | Module_compiled_without_lto name ->
      msg
        "Module %a@ carries@ no@ stored@ whole-program@ code@ \
         (it@ was@ built@ by@ a@ compiler@ configured@ without@ %a),@ \
         so@ %a@ is@ ignored@ for@ this@ link."
        Style.inline_code name
        Style.inline_code "--enable-lto"
